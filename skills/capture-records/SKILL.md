...
1. Choose two or three distinctive terms or short phrases from the note's topics, people, or situations. Call `search_records` with `types: ["journal"]` and `limit: 10` for each, making at most three searches. This tool matches literal text, not semantic similarity or Boolean queries. Use separate queries and, when useful, synonyms or terms in the journals' language without translating the original note. Apply date filters only when the user specifies a range.
