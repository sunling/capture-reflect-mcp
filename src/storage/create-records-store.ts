...
import { withRecordEditing } from "./record-editing.js";
...
export function createRecordsStore(config: AppConfig): RecordsStore {
  if (config.recordsStorage === "github") {
    ...
    return withRecordEditing(new GitHubRecordsStore(options), options);
  }
  return withRecordEditing(new LocalRecordsStore(config.recordsRepoPath), {
