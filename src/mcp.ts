...
export function createServer(
  ...
  server.registerTool(
    "capture_journal",
    {
      ...
      description:
        "Record a personal journal fragment in the user's original language. A new day creates YYYYMMDD.md with a dated heading and fragment headings; later fragments append to that file. If an older keyword-named journal exists for the date, append to it without renaming it. The filename never depends on the first topic. Do not ask the user for a keyword or pass one unless needed for a legacy client. Use for lived experiences, feelings, events and daily reflections, not technical debugging or ideas about the recording system (use capture_note). For a correction of existing journal text, or when the user explicitly identifies an existing journal path to update, use update_record after reading that file. Lightly edit for readability without inventing details or translating.",
      inputSchema: z.object({
        ...
        keyword: z.string().min(1).max(40).optional().describe("Legacy optional parameter for attachment naming only; ignored in the journal filename. Omit for normal journal captures."),
  ...
  server.registerTool(
    "update_record",
    {
      ...
      inputSchema: z.object({
        path: z.string().min(1).describe("Exact journals/YYYY/YYYYMM/YYYYMMDD.md (or legacy journal filename) or notes/YYYY/YYYYMM/filename.md path returned by a record search/read; never invent it."),
