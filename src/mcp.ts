...
import { registerGraphTool } from "./graph-tool.js";
...
export function createServer(
  ...
  const server = new McpServer(
    ...
    {
      instructions:
        "Minimize recording friction. FIRST decide the action (capture, retrieve, reflect/help, or update) from the user's explicit request and the current conversation, before choosing journal versus note. Once the user starts journaling, treat subsequent fragments as journal content until they change direction, without repeatedly asking for a type; this is session context, not a permanent default. Describing uncertainty, a meeting, existing notes, or a desire for clearer ideas inside a personal narrative is not automatically a request to search or solve it. If the user clearly asks to find notes, review records, or help prepare, do that; if they clearly ask to capture, capture without unrelated reads or advice. ONLY when capture versus help/retrieval remains genuinely ambiguous in an unestablished context and would materially change the action, ask one brief question BEFORE calling read or write tools, e.g. 'Would you like me to save this as today’s journal, or help you prepare for the meeting?' Honor the answer for the pending content without asking again. Do not ask a journal-versus-note folder question to resolve an action ambiguity. When capture is intended, choose capture_journal or capture_note by the user's intent, not keywords in a subsection. Explicit record type requests take priority: if the user asks to journal, write a diary, or save this as a journal, use capture_journal for their supplied entry even if it mentions a book, podcast, technical work, ideas, or something learned. If they explicitly ask for a note, use capture_note. An incidental mention of the word journal in a technical observation about the recording system is not an explicit journal request. If no type is specified, infer from the overall context: use capture_journal for lived experiences, feelings, events, or daily reflections; use capture_note for knowledge, encountered material, quotations, technical observations, measurements, product tests, design decisions, and ideas. If both destinations remain genuinely plausible, ask whether the user wants a journal or a note BEFORE calling any write tool; do not silently default to notes, and proceed using the user's answer. If they only say 'I would like to journal for today' without substantive content, invite them to share their entry and do not save that request as a record. For example 'Today, I would like to journal for today. After the checkup I ate breakfast and read Peter Hessler' is a journal, while '记录日记时 durationMs 是 6685' is a technical note unless explicitly requested as a journal. capture_journal creates a journal or automatically appends a new fragment for the same date. When a user explicitly wants to add to or correct an existing journal or note, use update_record: first search/read the exact target path, then append new material or replace one uniquely matching exact excerpt. Do not create a duplicate note because its path exists. Journal and note content may be in any language; preserve the original language, wording, uncertainty, code-switching, and original note text verbatim; never translate unless requested. Keep AI connections, reflections, and possible actions separate and explicitly labeled. Reply in the language of the current request. When the user says today or gives no date, omit the date argument to use the server-configured time zone; supply dates only when explicitly specified. After a successful capture or edit, present recordUrl as a clickable Markdown link when returned. Read records before reviews or questions about prior records; follow review-records and save_review for requested saved reviews. Treat prior review interpretations as distinct from original evidence. Use get_record_connections to inspect source-derived Markdown links and backlinks without modifying user records; IDs are optional and ordinary Markdown links remain the portable format. To switch GitHub accounts use get_github_account_switch_link; to set up GitHub, change repositories or update the time zone use get_github_setup_link, not the AI client's reconnect action.",
    },
  );
  ...
  server.registerTool(
    "capture_journal",
    {
      ...
      description:
        "Record a personal journal fragment in the user's original language. Automatically create a journal file for the record date or append a new headed fragment to the existing journal for that date. Choose the action first: during an ongoing journaling conversation, treat subsequent diary fragments as journal entries without asking for a folder repeatedly or searching just because they mention notes, uncertainty, or plans. If the user explicitly asks to journal or write a diary, choose this tool even when their entry mentions books, podcasts, knowledge, technical work, or ideas; honor their intended destination rather than reclassifying based on content. For an unspecified type, use for lived experiences, feelings, events and daily reflections, not technical debugging or ideas about the recording system (use capture_note). If a user only asks to start journaling without giving actual entry content, ask them to share it before calling this tool. If capture versus help remains genuinely ambiguous, ask one brief action question before any read or write; if only the record type is ambiguous, ask journal or note before a write. For a correction of existing journal text, or when the user explicitly identifies an existing journal path to update, use update_record after reading that file. Lightly edit for readability without inventing details or translating.",
      inputSchema: z.object({
  ...
  server.registerTool(
    "capture_note",
    {
      title: "Create a note",
      description:
        "Create a structured note in any language when the user explicitly requests a note, or when an unspecified record is clearly about an article, book, podcast, video, course, conversation, quotation, link, something learned, technical observation, debugging finding, measurement, product test, design decision, or idea. First determine whether the user wants capture at all: a journal narrative mentioning notes or uncertainty is not necessarily a request for advice or retrieval. Explicit journal or diary requests ALWAYS go to capture_journal, even when the user's daily account mentions a book or technical activity; do not turn the entire journal entry into a note. The recording system itself is normally a note subject even if its technical description contains journal or 日记, unless the user explicitly requests a journal. If both destinations are genuinely plausible and the user specified neither, ask whether to save as journal or note and wait before writing; do not silently default to this tool. Do not save a request to start journaling without entry content as a note. Use update_record for additions or corrections to an existing note; locate its exact path first. Preserve the user's original text verbatim in originalNote. Unless skipped, search_records with types: ['journal', 'note'], making at most three focused searches, and include at most three genuine relatedEntries total with verified dates, relative links, exact excerpts, and possible AI connections. Add furtherReflection or possibleActions only when useful; the server labels both as AI-generated and omits empty sections. If lookup fails, save the original and disclose the failure. The server generates consistent localized Markdown and keeps source details in the body rather than YAML. After saving show recordUrl if returned.",
      inputSchema: z.object({
        date: z.string().optional().describe("YYYY-MM-DD. Omit for today or unspecified date; the server uses its configured time zone. Supply only for an explicitly specified calendar date."),
        ...
        }).refine((value) => Object.values(value).some(Boolean), "Source must contain at least one field").optional().describe("Source must contain at least one field"),
  ...
  server.registerTool(
    "search_records",
    {
      ...
      description:
        "Search journal entries and notes for words or phrases in any language. Invoke when the user actually requests finding, recalling, or researching their records, not merely because a journal entry mentions past notes or a meeting. If capture versus retrieval remains genuinely unclear, ask one brief action question before searching; do not repeatedly clarify when an ongoing journal session already sets the intent. Set types: ['review'] to search saved reviews separately; date filters use save dates. Reviews contain interpretations, not independent evidence. Use the user's original search terms when possible. Use this when the user asks whether, when, or how they previously mentioned a person, topic, feeling, event, or idea.",
      inputSchema: z.object({
