...
describe("Standalone connection flow", () => {
  it.each(["", "unrelated=present; ", "first=one; second=two; "])("selects GitHub and a repository with cookie prefix %j before returning control to WorkOS", async (cookiePrefix) => {
    ...
    expect(pageBody).toContain("/auth/login?external_auth_id=ext_auth_test");
    expect(pageBody).toContain("GitHub controls which repositories");
    expect(pageBody).toContain("Use this repository");
    ...
    expect(savedPage).toContain("Returning to your chat");
    expect(savedPage).toContain("Return now");
